...
OpenRemoteHelpers.prototype.init = function (config) {
    ...
    OpenRemote = function(url, request) {
        ...
        switch(cmd) {
			...
			case "SwitchBinaryToggle":
				value = (zway.devices[N].instances[I].SwitchBinary.data.level.value) === "on" ? "off":"on");
				status = (zway.devices[N].instances[I].SwitchBinary.data.level.value) === "on" ? 0:255);
                zway.devices[N].instances[I].SwitchBinary.Set(value);
                return status;
