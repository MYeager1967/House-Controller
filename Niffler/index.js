	...
	var binderMethod;
        ...
        if(deviceType === 'doorlock'){
            binderMethod = function(type) {
				   ...
				   lastAlarmType = virtualDevice.get('metrics:alarmType')
				   alarmType = zway.devices[index].Alarm.data.V1event.alarmType.value
				   alarmLevel = zway.devices[index].Alarm.data.V1event.level.value
				   virtualDevice.set('metrics:alarmType', alarmType);
				   //virtualDevice.set('metrics:lastAlarmType', lastAlarmType);
				   virtualDevice.set('metrics:alarmLevel', alarmLevel);
				   console.log("Alarm Type = " + alarmType);
				   console.log("Alarm Level = " + alarmLevel);
				   console.log("Last Alarm Type = " + lastAlarmType);
                };
